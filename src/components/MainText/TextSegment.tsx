...
function pickContent(segment: Segment, lang: Language, mode: DisplayMode): string {
...
export function TextSegment({
  ...
  const hasParallels = segment.parallels.length > 0;
  ...
  return (
    ...
    >
      ...
      <span
        ...
        onMouseEnter={(e) => {
          if (hasParallels) (e.currentTarget as HTMLElement).style.filter = 'brightness(0.97)';
        }}
        onMouseLeave={(e) => {
      ...
      <AnnotationMarker count={annotationCount} onClick={() => onAnnotationClick(segment.id)} />
    </p>
  );
}
