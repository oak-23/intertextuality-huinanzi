...
import type { InlineParallel, Parallel } from '../../types';
...
function splitIntoSpans(
  ...
  type Boundary = { pos: number; type: 'start' | 'end'; parallel: InlineParallel };
...
export function MainText({ className }: MainTextProps) {
  ...
  const chapter = continuousText?.chapters.find((c) => c.id === state.activeChapterId) ?? null;

  const spans = useMemo(() => {
    if (!chapter) return [];
    const fullText = state.language === 'zh' ? chapter.text.zh : chapter.text.en;
  ...
  const handleHighlightClick = useCallback(
    (parallels: InlineParallel[], anchor: HTMLElement) => {
      ...
      if (parallels.length === 1) {
        ...
        setMultiParallels(unique.map((p) => ({
          textId: p.textId,
