...
import type { Chapter, ContinuousChapter } from '../types';

export interface UseChapterNavigationReturn {
  chapters: Array<Chapter | ContinuousChapter>;
  activeChapter: Chapter | ContinuousChapter | null;
...
export function useChapterNavigation(): UseChapterNavigationReturn {
  ...
  const chapters = useMemo(() => texts.getMainContinuousText()?.chapters ?? [], [texts]);
